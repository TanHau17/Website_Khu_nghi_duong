...
if (isset($_GET['dele'])) {
    ...
    if ($getlistpost == true)
        ...
        echo '<script>alert("Bài viết không thể xóa!!")</script>';
...
?>
<div
    style="background: #0056b3; height: 33px; color: #f2f2f2; font-size: 18px; padding-left: 10px; line-height: 33px; ">
...
<table style="width: 100%; margin: 5px;">
    ...
    while ($count < $sumcount) {
        ...
        echo '">
                <td>' . $count . '</td>
                <td style="text-align: center">' . $getlistpost[$count][1] . '</td>
                 <td style="text-align: center"><img style="width: 80px; height: 50px" src="../' . $getlistpost[$count][3] . '"></td>
          
               <td >' . $getlistpost[$count][4] . '</td>
              <td style="color: #b21f2d;  font-weight: 500; text-align: center"> 
             <button onClick="deleteitemn(this.id)" class="btn btn-danger   "  id="' . $getlistpost[$count][1] . '">Xóa</button>
            <a href="?select=edit&id=' . $getlistpost[$count][1] . '"><button  onClick="edititem(this.id)" id="' . $getlistpost[$count][1] . '" class="btn btn-primary">Chỉnh sửa</button></a>

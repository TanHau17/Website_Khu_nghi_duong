    ...
    public function getlistTraleviewing()
    {
        ...
        if ($count == 0) {
            ...
            while ($getdatabase = mysqli_fetch_assoc($data)) {
                $getdata[$dem][1] = $getdatabase['id'];
                $getdata[$dem][2] = $getdatabase['name'];
                $getdata[$dem][3] = $getdatabase['img'];
                $getdata[$dem][4] = $getdatabase['Address'];
                $getdata[$dem][5] = $getdatabase['price'];
                $getdata[$dem][6] = $getdatabase['contact'];
                $dem++;
            }
            return $getdata;
        }

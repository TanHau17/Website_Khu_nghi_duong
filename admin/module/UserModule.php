...
class UserModule
{
    ...
    public function editUserInformation($name,$address,$email,$pass, $phonenumber,$getid)
    {
        if(!$this->getconect){
            return false;
        }
        ...
        if($hl==true){
            $sql = " UPDATE user SET email='$email', password = '$pass', name='$name', mobno='$phonenumber', address='$address' where userid = $getid";
            echo $sql;
